"""Modified finetuning script for XLNet base model on the SST2's full sentences dataset."""
...
class SSTProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, data_num=None):
        ...
        tree_list = []
        with open(os.path.join(data_dir, 'train.txt')) as fobj:
            for line in fobj:
                tree_list.append(Tree.fromstring(line))

        examples_dict = {"Sentence": [], "FineLabel": []}
        for item in tree_list:
            examples_dict["Sentence"].append(' '.join(item.leaves()))
            examples_dict["FineLabel"].append(int(item.label()))

        train_dframe = pd.DataFrame(examples_dict)
        train_dframe = train_dframe[train_dframe["FineLabel"] != 2]
        train_dframe["Label"] = (train_dframe["FineLabel"] > 2).astype(int)
        train_dframe = train_dframe.drop("FineLabel", axis=1)
        return self._create_examples(train_dframe, "train", data_num=data_num)

    def get_dev_examples(self, data_dir, data_num=None):
        """Read testing file and create InputExamples for the values."""
        tree_list = []
        with open(os.path.join(data_dir, 'dev.txt')) as fobj:
            for line in fobj:
                tree_list.append(Tree.fromstring(line))
        ...
        dev_dframe = pd.DataFrame(examples_dict)
        dev_dframe = dev_dframe[dev_dframe["FineLabel"] != 2]
        dev_dframe["Label"] = (dev_dframe["FineLabel"] > 2).astype(int)
        dev_dframe = dev_dframe.drop("FineLabel", axis=1)
        return self._create_examples(dev_dframe, "dev", data_num=data_num)
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            with open(output_eval_file, "w") as writer:
                ...
                for key in sorted(result.keys()):
                    ...
                    writer.write("%s = %s\n" % (key, str(result[key])))
            print("Saving model")
            torch.save(model.module.state_dict(), os.path.join(args.output_dir, "sst-sentences-finetuned-xlnet-base-model_"+str(epoch)+".pth"))
