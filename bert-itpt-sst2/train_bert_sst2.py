...
from bert_model import BertConfig, BertForSequenceClassification
...
class SSTProcessor(DataProcessor):
    ...
    def get_all_examples(self, data_dir, data_num=None):
        ...
        sentiments = sentiments[(sentiments["sentiment values"] > 0.6000) | (sentiments["sentiment values"] < 0.40000)]
        # round the sentiment scores to binary 0 or 1
        sentiments["sentiment"] = np.around(sentiments["sentiment values"])

        final_dataset = train_data.merge(sentiments, how="left", on="phrase ids").dropna().drop("sentiment values", axis=1)
        final_dataset = final_dataset[final_dataset["sequence"].str.len() > 10]
